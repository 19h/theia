...
int IncrementalSfM::SelectInitialPairIndex() const {
  ...
  for (int idx = 0; idx < static_cast<int>(verified_pairs_.size()); ++idx) {
    ...
    int nH = 0;
    {
      ...
      cv::Mat mask;
      cv::Mat H = cv::findHomography(pts1, pts2, cv::RANSAC, cfg_.fund_ransac.inlier_threshold_px, mask);
      if (!H.empty() && !mask.empty()) nH = cv::countNonZero(mask);
    }

    const double homography_ratio = (nF > 0) ? (static_cast<double>(nH) / static_cast<double>(nF)) : 1.0;
    ...
    const double median_angle_deg = Median(angles_deg);
...
void IncrementalSfM::InitializeFromPair(ImageId i, ImageId j, const VerifiedPair& vp, Reconstruction* rec) {
  ...
  geometry::RelativePose rel = cheirality.pose;
  const double tnorm = rel.t.norm();
  PSYNTH_REQUIRE(tnorm > std::numeric_limits<double>::epsilon(), "InitializeFromPair: degenerate translation");
  rel.t /= tnorm;
  ...
  for (auto& track : rec->tracks.all_mut()) {
    ...
    const Eigen::Vector2d n1 = geometry::PixelToUndistortedNormalized(
        intr1, Eigen::Vector2d(oi->u_px, oi->v_px));
    const Eigen::Vector2d n2 = geometry::PixelToUndistortedNormalized(
        intr2, Eigen::Vector2d(oj->u_px, oj->v_px));
    ...
    const double e1 = geometry::ReprojectionErrorPx(rec->cameras[i], X, Eigen::Vector2d(oi->u_px, oi->v_px));
    const double e2 = geometry::ReprojectionErrorPx(rec->cameras[j], X, Eigen::Vector2d(oj->u_px, oj->v_px));
    if (e1 > cfg_.sfm.max_reprojection_error_px || e2 > cfg_.sfm.max_reprojection_error_px) continue;

    track.triangulated = true;
    track.xyz = X;

    const int u = std::max(0, std::min(color_img.cols - 1, static_cast<int>(std::lround(oi->u_px))));
    const int v = std::max(0, std::min(color_img.rows - 1, static_cast<int>(std::lround(oi->v_px))));
  ...
  std::cerr << "[psynth] init pair (" << i << "," << j << ") cameras=2 tracks=" << rec->tracks.all().size()
            << " triangulated=" << triangulated << "\n";
}

ImageId IncrementalSfM::SelectNextImage(const Reconstruction& rec, const std::vector<bool>& registered) const {
  const int N = dataset_.size();
  ImageId best = -1;
  int best_count = -1;
...
void IncrementalSfM::TriangulateNewTracks(Reconstruction* rec) {
  ...
  for (auto& track : rec->tracks.all_mut()) {
    ...
    for (size_t a = 0; a < obs_reg.size(); ++a) {
      for (size_t b = a + 1; b < obs_reg.size(); ++b) {
        const auto& ca = rec->cameras.at(obs_reg[a]->image_id);
        const auto& cb = rec->cameras.at(obs_reg[b]->image_id);
        const Eigen::Vector3d Ca = CameraCenterWorld(ca.pose);
        const Eigen::Vector3d Cb = CameraCenterWorld(cb.pose);
    ...
    const Eigen::Vector2d x1 = geometry::PixelToUndistortedNormalized(
        cam1.intr, Eigen::Vector2d(oa->u_px, oa->v_px));
    const Eigen::Vector2d x2 = geometry::PixelToUndistortedNormalized(
        cam2.intr, Eigen::Vector2d(ob->u_px, ob->v_px));
    ...
    cv::Mat img_color = dataset_.ReadColor(oa->image_id);
    const int u = std::max(0, std::min(img_color.cols - 1, static_cast<int>(std::lround(oa->u_px))));
    const int v = std::max(0, std::min(img_color.rows - 1, static_cast<int>(std::lround(oa->v_px))));
...
void IncrementalSfM::FilterTrackOutliers(Reconstruction* rec) {
  ...
  if (removed_obs > 0 || disabled_tracks > 0) {
    std::cerr << "[psynth] outlier filter: removed_obs=" << removed_obs
